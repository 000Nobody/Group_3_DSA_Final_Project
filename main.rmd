...
pacman::p_load(dplyr, ggplot2, glmnet, tidyverse, janitor, MASS, sjPlot, randomForest)
```
...
data_hs %>% ggplot(aes(x = salary_totals)) + 
  geom_histogram(bins = 30, fill = "blue", col = "black") + 
...
coef.sat <- coef(lasso_sat, s="lambda.min")
...
sat_test_result <- data.frame(predicted = predict(model.sat, data_hs.testing), actual = data_hs.testing$total_sat)
